...
void piece::set_type(char _type)
{
	this->type = _type;
}
...
{
	this->y = 18;
	this->x = piece_init_pos[this->type];
	this->form = 0;
	this->locked = false;
...
bool piece::is_colliding(int board[40][10], int x, int y)
{
	for (size_t _y = 0; _y < piece_form[this->type][this->form].size(); ++_y) {
		for (size_t _x = 0; _x < piece_form[this->type][this->form][_y].size(); ++_x) {
			if (piece_form[this->type][this->form][_y][_x] > 0) {
...
bool piece::is_lock_tspin(int board[40][10])
{
...
void piece::try_left(int board[40][10])
{
	if (!is_colliding(board, this->x - 1, this->y)) { this->x--; this->on_floor_timer = 0.0; }
}

void piece::try_right(int board[40][10])
{
	if (!is_colliding(board, this->x + 1, this->y)) { this->x++; this->on_floor_timer = 0.0; }
}

void piece::try_down(int board[40][10])
{
	if (!is_colliding(board, this->x, this->y + 1)) { this->y++; }
}

void piece::try_hard_drop(int board[40][10])
{
	while (!is_colliding(board, this->x, this->y + 1)) { this->y++; }
	this->lock();
}

void piece::try_rotate(int board[40][10], int dir)
{
	/*
	...
	*/
	...
	this->form = (this->form + dir) % (piece_form[this->get_type()].size());
	int k = 0;
	...
	if (this->get_type() != 'O') {
		if (this->is_colliding(board, this->x, this->y)) {
			int c = 0;
			for (int i = 0; i < 4; i++) {
				if (!this->is_colliding(board, this->x + piece_srs_data[this->get_type()][k][i][0], this->y + piece_srs_data[this->get_type()][k][i][1])) {
					this->x += piece_srs_data[this->get_type()][k][i][0];
					this->y += piece_srs_data[this->get_type()][k][i][1];
					break;
				}
				c++;
			}
...
void piece::update_lock(double dt, int board[40][10])
{
...
void piece::update(double dt, int board[40][10])
{
	// das
